...
class PermissionParts private constructor(val parts: List<String>, val currentIndex: Int) {
    constructor(vararg parts: String) : this(parts.toList(), 0)
    constructor(parts: List<String>) : this(parts.toList(), 0)
    val fullId: String get() = parts.joinToString("/") { URLEncoder.encode(it, StandardCharsets.UTF_8) }
    fun next() = next(1)
