...
class JSCodeCompletionMenuUI(val ccmenu: CodeCompletionMenu, val editor: JsEditorComponent) {
    fun updateBounds() {
        ...
        val anchorRelativeBounds = anchorAbsoluteBounds.relativeTo(editor.getMainLayer()?.getAbsoluteBounds() ?: ZERO_BOUNDS)
        val patternElement = ccContainerElement.descendants().filterIsInstance<HTMLElement>().first { it.classList.contains("ccmenu-pattern") }
        ...
        val caretElement = ccContainerElement.descendants().filterIsInstance<HTMLElement>().first { it.classList.contains("caret") }
