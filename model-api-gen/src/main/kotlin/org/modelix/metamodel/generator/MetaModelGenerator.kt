...
private val jsNameAnnotation = ClassName("kotlin.js", "JsName")
...
class MetaModelGenerator(
    ...
    var kotlinTargetPlatform: KotlinTargetPlatform = KotlinTargetPlatform.Common

    private val headerComment = "\ngenerated by modelix model-api-gen \n"

    private fun <T : Annotatable.Builder<*>> T.jsExportIfEnabled(name: String? = null) = also {
        if (kotlinTargetPlatform != KotlinTargetPlatform.JVM) {
            addAnnotation(jsExportAnnotation)
            if (name != null) {
                jsNameIfEnabled(name)
            }
        }
    }

    private fun <T : Annotatable.Builder<*>> T.jsNameIfEnabled(name: String) = also {
        if (kotlinTargetPlatform != KotlinTargetPlatform.JVM) {
            addAnnotation(AnnotationSpec.builder(jsNameAnnotation).addMember("%S", name).build())
        }
    }

    private fun <T : Annotatable.Builder<*>> T.jsExportIgnoreIfEnabled() = also {
        if (kotlinTargetPlatform != KotlinTargetPlatform.JVM) {
    ...
    private fun generateConceptFile(concept: ProcessedConcept) {
        FileSpec.builder(concept.language.name, concept.name)
            .addFileComment(headerComment)
            .addType(generateUntypedConcept(concept))
            .addTypeAlias(TypeAliasSpec.builder(concept.conceptTypeAliasName(), concept.conceptWrapperInterfaceType()).build())
            .addType(generateTypedConceptInterface(concept))
//            .addType(generateConceptWrapperImpl(concept))
            .addType(generateTypedNodeInterface(concept))
            .addFunction(
                FunSpec.builder("_isOfConcept_${concept.name}")
                    .jsExportIfEnabled()
                    .addParameter("node", ITypedNode::class)
                    .returns(Boolean::class)
                    .addStatement("return node·is·%T", concept.nodeWrapperInterfaceType())
                    .build(),
            )
            .addType(generateTypedNodeImpl(concept))
            .apply {
    ...
    private fun generateUntypedConcept(concept: ProcessedConcept): TypeSpec {
    ...
    private fun generateTypedConceptInterface(concept: ProcessedConcept): TypeSpec {
    ...
    private fun generateTypedNodeImpl(concept: ProcessedConcept): TypeSpec {
    ...
    private fun generateTypedNodeInterface(concept: ProcessedConcept): TypeSpec {
