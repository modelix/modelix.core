            ...
            if (useNodeProxy) {
                addFunction(
                    FunSpec.builder(GeneratedConcept<*, *>::wrap.name)
                        .returns(concept.nodeWrapperInterfaceType())
                        .addModifiers(KModifier.OVERRIDE)
                        .addParameter("node", INode::class)
                        .addStatement("return %M(node, typed())", MemberName("org.modelix.metamodel", "createNodeProxy"))
                        .build()
                )
            } else {
                addFunction(
