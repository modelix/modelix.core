...
class CompositeNodeResolveContextTest {
    ...
    fun resolveWhenEmpty() {
        ...
        assertEquals(null, instance.resolveNode(LocalPNodeReference(123)))
    ...
    fun resolveWhenNotEmpty() {
        val instance = CompositeArea(
            MyArea(mapOf(LocalPNodeReference(123) to MyNode("foo"))),
            MyArea(mapOf(LocalPNodeReference(456) to MyNode("bar"))),
        )
        assertEquals(MyNode("foo"), instance.resolveOriginalNode(LocalPNodeReference(123)))
        assertEquals(MyNode("bar"), instance.resolveOriginalNode(LocalPNodeReference(456)))
        assertEquals(null, instance.resolveNode(LocalPNodeReference(789)))
        assertEquals(null, instance.resolveNode(MyNodeReference()))
    }
}
