...
class SetConceptOp(val nodeId: Long, val concept: IConceptReference?) : AbstractOperation() {
    ...
    override fun captureIntend(tree: ITree, store: IDeserializingKeyValueStore) = Intend()
    ...
    inner class Intend : IOperationIntend {
        ...
        override fun restoreIntend(tree: ITree): List<IOperation> {
            return if (tree.containsNode(nodeId)) listOf(this@SetConceptOp) else listOf(NoOp())
        }
    }
}
