...
data class CPVersion(
    ...
    val treeHash: ObjectReference<CPTree>,
    ...
    override fun serialize(): String {
        ...
        return longToHex(id) +
            s + escape(time) +
            s + escape(author) +
            s + nullAsEmptyString(treeHash.getHashString()) +
    ...
    override fun getDeserializer(): (String) -> IObjectData = DESERIALIZER
            ...
            try {
                ...
                if (parts.size == 9) {
                    ...
                    val data = CPVersion(
                        ...
                        unescape(parts[1]),
                        unescape(parts[2]),
                        treeHash = ObjectReference(
                            checkNotNull(emptyStringAsNull(parts[3])) { "Tree hash empty in $input" },
                            CPTree.DESERIALIZER,
                        ),
                        previousVersion = null,
                        originalVersion = null,
                        baseVersion = emptyStringAsNull(parts[4])?.let { ObjectReference(it, DESERIALIZER) },
                        mergedVersion1 = emptyStringAsNull(parts[5])?.let { ObjectReference(it, DESERIALIZER) },
                        mergedVersion2 = emptyStringAsNull(parts[6])?.let { ObjectReference(it, DESERIALIZER) },
                        operations = ops,
                        operationsHash = opsHash?.let { ObjectReference(it, OperationsList.DESERIALIZER) },
                    ...
                    val data = CPVersion(
                        ...
                        previousVersion = emptyStringAsNull(parts[4])?.let { ObjectReference(it, DESERIALIZER) },
                        ...
                        mergedVersion2 = null,
                        ops,
                        opsHash?.let { ObjectReference(it, OperationsList.DESERIALIZER) },
                        numOps,
                    )
                    return data
                }
    ...
    fun withOperations(operations: List<IOperation>): CPVersion {
        ...
        return if (localizedOps.size <= INLINED_OPS_LIMIT) {
            ...
            val opsList = OperationsList.of(localizedOps)
            copy(
                operations = null,
                operationsHash = ObjectReference(opsList),
                numberOfOperations = localizedOps.size,
            )
        }
    }
