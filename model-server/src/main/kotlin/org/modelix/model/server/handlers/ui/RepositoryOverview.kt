...
class RepositoryOverview(private val repoManager: IRepositoriesManager) {
    ...
    private fun FlowContent.buildMainPage(call: ApplicationCall) {
        ...
        val repositories = repoManager.getRepositories().filter { call.hasPermission("repository", it.id, "list") }
            ...
            table {
                ...
                tbody {
                    for (repository in repositories) {
                        val branches = repoManager.getBranches(repository).filter { call.hasPermission("repository", it.repositoryId.id, "branch", it.branchName, "list") }
