...
class MutableGitFS(
    ...
    fun createCommit(message: String, author: String?): ObjectId {
        ...
        return gitRepo.newObjectInserter().use { it.insert(commit) }
    }

    private fun writeTree(): ObjectId {
        val dirCache = DirCache.newInCore()
        val builder: DirCacheBuilder = dirCache.builder()
        root.loadEntries(builder, null)
        builder.finish()
        return gitRepo.newObjectInserter().use { dirCache.writeTree(it) }
    }
}
