...
class SetPropertyActionGroup : ActionGroup() {
    ...
    override fun getChildren(event: AnActionEvent?): Array<AnAction> {
        if (event?.getTreeNode() is CloudNodeTreeNode) {
            ...
            if (node is PNodeAdapter) {
                ...
                val properties = concept.getAllProperties()
                return properties.sortedBy { it.getSimpleName() }.map { SetPropertyAction(node, it) }
