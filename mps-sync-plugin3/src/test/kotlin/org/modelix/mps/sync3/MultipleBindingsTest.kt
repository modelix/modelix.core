...
class MultipleBindingsTest : ProjectSyncTestBase() {
     ...
     * second repository.
     * That library repository may or may not contain a project. In any case, only the modules should be synchronized
     * and added to the MPS project (to make them visible to the user). When synchronizing back to the primary
     * repository, the library modules need to be ignored and not added to the project on the server.
     */
    fun `test library repository`() = runWithModelServer { port ->
